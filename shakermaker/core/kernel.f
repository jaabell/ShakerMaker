...
c Input:
c       k       wavenumber
c       /model/ velocity model (see model.h) !common /model/mb,stype,src,rcv,updn,ka,kb,d,rho,mu,xi,si
c Output:
...
c***************************************************************
      subroutine kernel(k,u,mb,stype,src,rcv,updn,ka,kb,d,rho,mu,xi,si)
      IMPLICIT NONE
      include   'layer.h'
      include   'model.h'
      integer   i, j
...
c propagation - start from the bottom
      do j = mb, 1, -1
         call layerParameter(k,j,mb,stype,src,rcv,updn,ka,kb,d,rho,mu,xi,si,kd,mu2,exa,exb,ra,rb,Ca,Cb,Ya,Xa,Yb,Xb,r,r1,y,y1)
         if ( j.EQ.mb .AND. d(j).LT.epsilon ) then
            call initialG(g,kd,mu2,exa,exb,ra,rb,Ca,Cb,Ya,Xa,Yb,Xb,r,r1,y,y1)
         else if ( j.EQ.1 .AND. d(1).LT.epsilon ) then
            call eVector(e,kd,mu2,exa,exb,ra,rb,Ca,Cb,Ya,Xa,Yb,Xb,r,r1,y,y1)
            exit
         else
            call compoundMatrix(c,kd,mu2,exa,exb,ra,rb,Ca,Cb,Ya,Xa,Yb,Xb,r,r1,y,y1)
            call propagateG(c, g)
         endif
         if ( j.EQ.src ) then
            call separatS(ss,mb,stype,src,rcv,updn,ka,kb,d,rho,mu,xi,si,kd,mu2,exa,exb,ra,rb,Ca,Cb,Ya,Xa,Yb,Xb,r,r1,y,y1)
            call initialZ(ss, g, z)
         endif
         if ( j.LT.src ) then
            if ( j.GE.rcv ) then
               call haskellMatrix(a,kd,mu2,exa,exb,ra,rb,Ca,Cb,Ya,Xa,Yb,Xb,r,r1,y,y1)
               call propagateZ(a, z)
            else
               call propagateB(c, b)
            endif
...
c displacement kernels at the receiver
      ...
      subroutine separatS(ss,mb,stype,src,rcv,updn,ka,kb,d,rho,mu,xi,si,kd,mu2,exa,exb,ra,rb,Ca,Cb,Ya,Xa,Yb,Xb,r,r1,y,y1)
      IMPLICIT NONE
      include   'layer.h'
      include   'model.h'
      integer i, j, ii, jj
